...
const index = require('../shared/consola.cf75fe3c.cjs');
require('../shared/consola.0c6976c3.cjs');
require('node:util');
require('node:path');
...
const tty__default = /*#__PURE__*/_interopDefaultCompat(tty$1);
...
const symbol = (state) => {
  switch (state) {
    case "initial":
    case "active":
      return index.cyan(S_STEP_ACTIVE);
    case "cancel":
      return index.red(S_STEP_CANCEL);
    case "error":
      return index.yellow(S_STEP_ERROR);
    case "submit":
      return index.green(S_STEP_SUBMIT);
  }
};
const text = (opts) => {
  return new oD({
    ...
    render() {
      const title = `${index.gray(S_BAR)}
${symbol(this.state)} ${opts.message}
`;
      const placeholder = opts.placeholder ? index.inverse(opts.placeholder[0]) + index.dim(opts.placeholder.slice(1)) : index.inverse(index.hidden("_"));
...
${index.yellow(S_BAR_END)} ${index.yellow(
            this.error
          )}
`;
        case "submit":
          return `${title}${index.gray(S_BAR)} ${index.dim(
            this.value || opts.placeholder
          )}`;
        case "cancel":
          return `${title}${index.gray(S_BAR)} ${index.strikethrough(
            index.dim(this.value ?? "")
          )}${this.value?.trim() ? "\n" + index.gray(S_BAR) : ""}`;
        default:
          return `${title}${index.cyan(S_BAR)} ${value}
${index.cyan(
            S_BAR_END
          )}
`;
...
`;
      ...
      switch (this.state) {
        case "submit":
          return `${title}${index.gray(S_BAR)} ${index.dim(value)}`;
        case "cancel":
          return `${title}${index.gray(S_BAR)} ${index.strikethrough(
            index.dim(value)
          )}
${index.gray(S_BAR)}`;
        default: {
          return `${title}${index.cyan(S_BAR)} ${this.value ? `${index.green(S_RADIO_ACTIVE)} ${active}` : `${index.dim(S_RADIO_INACTIVE)} ${index.dim(active)}`} ${index.dim("/")} ${!this.value ? `${index.green(S_RADIO_ACTIVE)} ${inactive}` : `${index.dim(S_RADIO_INACTIVE)} ${index.dim(inactive)}`}
${index.cyan(S_BAR_END)}
`;
        }
      }
    }
...
const select = (opts) => {
  const opt = (option, state) => {
    ...
    switch (state) {
      case "active": {
        return `${index.green(S_RADIO_ACTIVE)} ${label} ${option.hint ? index.dim(`(${option.hint})`) : ""}`;
      }
      case "selected": {
        return `${index.dim(label)}`;
      }
      case "cancelled": {
        return `${index.strikethrough(index.dim(label))}`;
      }
    }
    return `${index.dim(S_RADIO_INACTIVE)} ${index.dim(label)}`;
  };
  return new ED({
...
`;
      switch (this.state) {
        case "submit":
          return `${title}${index.gray(S_BAR)} ${opt(
...
${index.gray(S_BAR)}`;
        default: {
          return `${title}${index.cyan(S_BAR)} ${this.options.map(
...
const multiselect = (opts) => {
  const opt = (option, state) => {
    ...
    switch (state) {
      case "active": {
        return `${index.cyan(S_CHECKBOX_ACTIVE)} ${label} ${option.hint ? index.dim(`(${option.hint})`) : ""}`;
      }
      case "selected": {
        return `${index.green(S_CHECKBOX_SELECTED)} ${index.dim(label)}`;
      }
      case "cancelled": {
        return `${index.strikethrough(index.dim(label))}`;
      }
      case "active-selected": {
        return `${index.green(S_CHECKBOX_SELECTED)} ${label} ${option.hint ? index.dim(`(${option.hint})`) : ""}`;
      }
      case "submitted": {
        return `${index.dim(label)}`;
      }
    }
    return `${index.dim(S_CHECKBOX_INACTIVE)} ${index.dim(label)}`;
  };
  return new iD({
    options: opts.options,
...
${index.reset(
          index.dim(
            `Press ${index.gray(
              index.bgWhite(index.inverse(" space "))
            )} to select, ${index.gray(
              index.bgWhite(index.inverse(" enter "))
            )} to submit`
          )
        )}`;
      }
...
`;
      switch (this.state) {
        case "submit": {
          return `${title}${index.gray(S_BAR)} ${this.options.filter(({ value }) => this.value.includes(value)).map((option) => opt(option, "submitted")).join(index.dim(", ")) || index.dim("none")}`;
        }
        case "cancel": {
          const label = this.options.filter(({ value }) => this.value.includes(value)).map((option) => opt(option, "cancelled")).join(index.dim(", "));
          return `${title}${index.gray(S_BAR)} ${label.trim() ? `${label}
${index.gray(S_BAR)}` : ""}`;
        }
        case "error": {
          const footer = this.error.split("\n").map(
            (ln, i) => i === 0 ? `${index.yellow(S_BAR_END)} ${index.yellow(ln)}` : `   ${ln}`
          ).join("\n");
          return title + index.yellow(S_BAR) + "  " + this.options.map((option, i) => {
...
${index.yellow(S_BAR)}  `) + "\n" + footer + "\n";
        ...
        default: {
          return `${title}${index.cyan(S_BAR)} ${this.options.map((option, i) => {
            const selected = this.value.includes(option.value);
            const active = i === this.cursor;
            if (active && selected) {
              return opt(option, "active-selected");
